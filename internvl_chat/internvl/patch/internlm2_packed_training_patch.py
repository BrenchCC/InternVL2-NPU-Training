...
class InternLM2FlashAttention2ForPackedTraining(InternLM2FlashAttention2):
    ...
    def _flash_attention_forward(
        ...
        '''
        atten_mask_npu= torch.from_numpy(np.triu(np.ones([max_seqlen, max_seqlen]), k=1)).bool().to(device)
        head_num = query_states.shape[1]
        attn_output = torch_npu.npu_fusion_attetion(
            q=query_states,
            k=key_states,
            v=value_states,
            head_num=head_num,
