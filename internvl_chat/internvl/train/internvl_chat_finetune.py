...
def main():
    ...
    # NOTE modified by brench~chenbaizheng
    if model_args.model_name_or_path is not None:
        ...
        if config.llm_config.model_type == 'internlm2':
            ...
            # config.llm_config.attn_implementation = 'eager'  # for InternLM with ascend npu
            # config.llm_config._attn_implementation = 'sdpa'  # for InternLM
            logger.info('Using eager attention implementation for InternLM')
        else:
            config.llm_config._attn_implementation = 'flash_attention_2'  # for LLaMA
            # config.llm_config.attn_implementation = 'eager'  # for LLaMA
            # config.llm_config._attn_implementation = 'sdpa'  # for LLaMA
            logger.info('Using flash_attention_2 for LLaMA')
            # logger.info('Using eager attention implementation for LLaMA')
        ...
        if llm_config.model_type == 'internlm2':
            ...
            # llm_config.attn_implementation = 'eager'  # for InternLM with ascend npu
            # llm_config.attn_implementation = 'sdpa'  # for InternLM
            # logger.info('Using eager attention implementation for InternLM')
            logger.info('Using flash_attention_2 for InternLM')
        else:
            model_type = AutoModelForCausalLM
            llm_config.attn_implementation = 'flash_attention_2'  # for LLaMA
            # llm_config.attn_implementation = 'eager'  # for LLaMA with ascend npu
            # llm_config.attn_implementation = 'sdpa'  # for LLaMA
            logger.info('Using eager attention implementation for LLaMA')
            # logger.info('Using flash_attention_2 for InternLM')
